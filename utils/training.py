...
def gen_loss_seg_nomorph(network_eval_batch, label_placeholder):
	# Create the 2 bins
	less_g = tf.less(label_placeholder,200)
	greater_g = tf.greater(label_placeholder,99)
	gray = less_g & greater_g
	val_gray = tf.to_float(gray)*0.5
	white = tf.greater(label_placeholder,199)
	val_white = tf.to_float(white)
	mouse_label = val_gray+val_white
...
def gen_train_op_sgd(loss, train_size, batch_size, global_step, init_learn_rate = 1e-4, num_epochs_per_decay = 50, const_learn_rate = False):
	num_batches_per_epoch = ((train_size) / batch_size)
	decay_steps = int(num_batches_per_epoch * num_epochs_per_decay)
