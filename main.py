...
def main(argv):
	...
	possible_models = {x[0]:x[1] for x in possible_models if inspect.isfunction(getattr(models, x[0]))} #get all the functions in the module of models
	...
	parser.add_argument('--net_type', help='Type of network model (default segmentation)', choices=['ellreg','segellreg','binned','seg'], default='seg')
	parser.add_argument('--batch_size', help='Batch size of the network (default 16)', type=int, default=16)
	...
	parser_train.add_argument('--network_to_restore', help='Network checkpoint to restore') #已训练好的模型参量的保存路径及文件名
	parser_train.add_argument('--n_reader_threads', help='Number of CPU threads for fetching data (default 3)', default=3, type=int)
	parser_train.add_argument('--log_dir', help='Log folder', default='.') # defalt: current directory
	parser_train.add_argument('--train_list', help='File containing list of identifiers for training', required=True) #list是如何生成的
	...
	parser_train.add_argument('--start_learn_rate', help='Initial learning rate for training (default 5e-7)', type=float, default=5e-7) #如何改变学习率
	...
	arg_dict['model_construct_function'] = possible_models[args.model] #报错，无此参数
